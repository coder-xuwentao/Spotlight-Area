...
chrome.storage.onChanged.addListener(function handleBlurOrDarknessChange(changes, area) {
...
chrome.runtime.onMessage.addListener(function handleSwitchSpotLightFromPopup(request, sender, sendResponse) {
...
async function initConfigToMask() {
  const { deGreeOfBlur, deGreeOfDarkness } = await chrome.storage.local.get(['deGreeOfBlur', 'deGreeOfDarkness'])
...
function startSelectLightArea() {
  ...
  // 监听点击确认高亮区域
  document.addEventListener("mousedown", handleMouseDownToSelect, { once: true, capture: true });
...
function setDeGreeOfBlurVar(value) {
  document.documentElement.style.setProperty("--slaDeGreeOfBlur", `${Number(value)}px`);
}

// 设置暗度
function setDeGreeOfDarknessVar(value) {
  document.documentElement.style.setProperty("--slaDeGreeOfDark", `${Number(value)/100}`);
}

// 本来想用在handleScroll，但体验差算了
