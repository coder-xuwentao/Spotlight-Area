...
chrome.runtime.onInstalled.addListener(async function initState() {
  ...
  async function initState() {
    ...
    if (deGreeOfBlur === undefined) {
      chrome.storage.local.set({ deGreeOfBlur: DEFAULT_BLUR });
    }
    if (deGreeOfDarkness === undefined) {
      chrome.storage.local.set({ deGreeOfDarkness: DEFAULT_DARKNESS });
    }
...
chrome.tabs.onActivated.addListener(async (activeInfo) => {
  ...
  async function resetSpotlightSwitchDepsOnContent() {
    ...
    const hasSpotLight = await chrome.tabs.sendMessage(activeInfo.tabId, {
      action: "tellMeHasSpotLight",
    });
...
// 处理来自content的action
chrome.runtime.onMessage.addListener(function handleActionFromContent(
  request,
  sender,
  sendResponse
) {
